...
const PokemonFr = ({
  ...
  typeLabels,
  priority = false,
}: IBasicPokemon & { typeLabels: Record<string, string>; priority?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      {evolvesFrom && (
        <span className={styles.evoBadge} title={`Évolue de ${evolvesFrom.name}`}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={evoImageUrl} alt={`Illustration de ${evolvesFrom.name}`} data-sprite="" loading="lazy" />
        </span>
      )}
      ...
      <div className={styles.head}>
        ...
        <span className={styles.hp}>{`${FR_STAT_LABELS["Hp"]} ${hp}`}</span>
      </div>
